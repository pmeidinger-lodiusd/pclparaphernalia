...
namespace PCLParaphernalia
{
    /// <summary>
    ///
    /// <para>Class handles PCL downloadable soft font headers.</para>
    ...
    class PrnParseFontHddrPCL
    {
        ...
        private PCLFontFormat _hddrFormat;
                                       ...
                                       DataTable table)
        {
            ...
            if (_nextStage == Stage.Start)
            {
                if (bufRem < 2)
                {
                    ...
                    contType = PrnParseConstants.ContType.PCLFontHddr;
            ...
            if (_nextStage == Stage.ShowData)
            {
                ...
                if ((_hddrFormat == PCLFontFormat.IntellifontBound)
                                         ||
                    (_hddrFormat == PCLFontFormat.IntellifontUnbound))
                {
                    ProcessGlobalData(ref bufRem,
                                      ref bufOffset);
                }
                else if ((_hddrFormat == PCLFontFormat.TrueType)
                                        ||
                         (_hddrFormat == PCLFontFormat.Universal))
                {
                    largeSegs = _hddrFormat == PCLFontFormat.Universal;

                    bool continuation = ParseSegs.ProcessSegData(_buf,
                                                  _fileOffset,
                                                  true,
                                                  _firstSeg,
                                                  largeSegs,
                    ...
                    {
                        if (validSegs)
                            _nextStage = Stage.ShowCopyright;
                        else
                            _nextStage = Stage.BadSeqA;
                    }
                }
            ...
            if (_nextStage == Stage.ShowCopyright)
            {
                //------------------------------------------------------------//
                //                                                            //
                ...
                {
                    ProcessCopyrightData(ref bufRem, ref bufOffset);
                }
            }

            if (_nextStage == Stage.ShowChecksum)
            {
                ProcessChecksum(ref bufRem, ref bufOffset);
            }
        ...
        private void ProcessCopyrightData(ref int bufRem, ref int bufOffset)
        {
        ...
        private void ProcessDescriptor(ref int bufLen, ref int bufOffset)
        {
            ASCIIEncoding ascii = new ASCIIEncoding();
            ...
            _hddrFormat = (PCLFontFormat)_buf[bufOffset + 2];

            if ((_hddrFormat == PCLFontFormat.Bitmap)
                               ||
                (_hddrFormat == PCLFontFormat.BitmapResSpec))
            {
                //------------------------------------------------------------//
                //                                                            //
            ...
            else if ((_hddrFormat == PCLFontFormat.IntellifontBound)
                                   ||
                     (_hddrFormat == PCLFontFormat.IntellifontUnbound))
            {
                //------------------------------------------------------------//
                ...
                else
                {
                    ...
                    if ((_hddrDescLen + _hddrDataLen + _hddrCpyrLen + 2) != _hddrLen)
                    {
                        _validHddr = false;
            ...
            else
            {
                ...
                else if ((_hddrFormat == PCLFontFormat.BitmapResSpec)
                                            &&
                         (_hddrDescLen > 65))
                {
                ...
                {
                    case 0:
                        itemDesc = PCLSymSetTypes.GetDescStd((int)PCLSymSetTypes.Index.Bound_7bit);
                        break;

                    case 1:
                        itemDesc = PCLSymSetTypes.GetDescStd((int)PCLSymSetTypes.Index.Bound_8bit);
                        break;

                    case 2:
                        itemDesc = PCLSymSetTypes.GetDescStd((int)PCLSymSetTypes.Index.Bound_PC8);
                        break;

                    case 3:
                        itemDesc = PCLSymSetTypes.GetDescStd((int)PCLSymSetTypes.Index.Bound_16bit);
                        break;

                    case 10:
                        itemDesc = PCLSymSetTypes.GetDescStd((int)PCLSymSetTypes.Index.Unbound_MSL);
                        _boundFont = false;
                        break;

                    case 11:
                        itemDesc = PCLSymSetTypes.GetDescStd((int)PCLSymSetTypes.Index.Unbound_Unicode);
                        _boundFont = false;
                        break;
                ...
                if (((_hddrFormat == PCLFontFormat.TrueType)
                                       ||
                     (_hddrFormat == PCLFontFormat.Universal))
                                       &&
                    (_hddrDescLen > 71))
                ...
                if ((_hddrFormat == PCLFontFormat.IntellifontUnbound)
                                        &&
                    (_hddrDescLen > 85))
                {
        ...
        private void ProcessGlobalData(ref int bufRem, ref int bufOffset)
        {
            PrnParseConstants.ContType contType;
